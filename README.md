...
- R² remains negative because the exact minute at which the High/Low occur is not identifiable from OHLC + event schedule alone. The bridge pins the level, not the time.
- KS is the primary fidelity metric for synthetic markets; R² on a random path is a harsh baseline.
- Regime signal is wasted on buy/hold/sell. To exploit regimes, extend the action space to include quote width and position size (e.g., size ∈ {0, 5, 10, 25%} × side).
- Volume is not yet used in the generator. Incorporating volume into Hawkes intensity is the next research direction.
